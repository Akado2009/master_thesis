...
std::map<std::string, std::vector<Mutation* > > parse_mutations_file(std::string filename, bool aminoacid, Data* data, char flag) {
    ...
    for(std::string line; std::getline(mutations, line);) {
        if (line.rfind("Chromosome", 0) != 0) {
            ...
            std::string name = split_string(parsed_line[8], '.')[0];
            if (result.find(name) == result.end()) {
                result[name] = std::vector<Mutation *>{};
            }
            if (!aminoacid) {
                result[name].push_back(
                        construct_mutation(std::stoi(parsed_line[6]), parsed_line[8][0], parsed_line[9][0]));
            } else {
                ...
                int i = 0;
                if (is_valid_mutation(parsed_line[8])) {
                    std::vector<std::string> preparsed_line = split_string(parsed_line[8], ' ');
...
//                    std::cout << position << " " << first << " " << second << std::endl;
                    if (position != "") {
                        result[name].push_back(
                                construct_mutation(std::stoi(position), aa_dict[first], aa_dict[second]));
                    }
                }
...
//            >NM_000299_hg38 2244 chr1:201283703-201328836+
            ...
            std::vector<Mutation *> path_possible_mutations = path_mutations[current_name];
            std::vector<Mutation *> benign_possible_mutations = benign_mutations[current_name];
...
int main() {
    ...
    std::map<std::string, std::vector<Mutation* > > benign_mutations = parse_mutations_file("data/results/benign.tab", aminoacid, info, 'b');
    ...
    parse_alignment("data/alignments/alignments_16", path_mutations, benign_mutations, support_info, aminoacid, export_alignments, info, true);
//    output_result();
    std::cout << info->patho_one << " " << info->patho_more << std::endl;
    std::cout << info->benign_one << " " << info->benign_more << std::endl;
